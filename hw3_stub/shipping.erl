...
get_occupied_docks(Shipping_State, Port_ID) ->
    ...
    DD = lists:filter(fun ({P, D, S}) -> P =:= Port_ID end, Shipping_State#shipping_state.ship_locations),
    lists:map(fun ({P, D, S}) -> D end, DD).
...
get_container_weight(Shipping_State, Container_IDs) ->
    case Container_IDs of
      [] -> 0;
      [_H | _T] ->  lists:foldl(fun (CID, Sum) ->  (get_container(Shipping_State, CID))#container.weight + Sum end, 0,  Container_IDs);
      _E -> error
    end.
...
get_ship_weight(Shipping_State, Ship_ID) ->
  case maps:find(Ship_ID, Shipping_State#shipping_state.ship_inventory) of
    {ok, E} -> get_container_weight(Shipping_State, E);
    _E -> error
  end.
...
load_ship(Shipping_State, Ship_ID, Container_IDs) ->
        ...
        case(length(SHINV) + length(Container_IDs) =< SCap) and is_sublist(PINV, Container_IDs) of
          ...
          NEW_PINV = lists:filter(fun (X) -> not lists:member(X, Container_IDs) end, PINV),
            #shipping_state{
                        ...
                        port_inventory = maps:put(Container_IDs, NEW_PINV, Shipping_State#shipping_state.port_inventory)
                        };
          false ->
            error
          end;
    ...
    %     {ok, SHINV} = maps:find(Ship_ID Shipping_State#shipping_state.ship_inventory),
    %     {ok, PINV} = maps:find(Port, Shipping_State#shipping_state.port_inventory),
    %     case (length(SCap) + 1 =< SCap and lists:member(H, PINV)) of
    %       true ->
    %       USHINV = SHINV++H,
    %       UPINV = lists:filter(fun (X) -> X =/= H end, PINV),
    %       NShip = #shipping_state{
    %             ships = Shipping_State#shipping_state.ships,
    %             containers = Shipping_State#shipping_state.containers,
    %             ports = Shipping_State#shipping_state.ports,
    %             ship_locations = Shipping_State#shipping_state.ship_locations,
    %             ship_inventory = USHINV ,
    %             port_inventory = UPINV
    %             },
    %       false ->
    %          error
    %     end;
...
unload_ship(Shipping_State, Ship_ID, Container_IDs) ->
  ...
  case(length(PINV) + length(Container_IDs) =< PortCap) and is_sublist(SHINV, Container_IDs) of
      true ->
        ...
        NEW_PINV =  lists:merge(PINV, Container_IDs),
          #shipping_state{
                      ships = Shipping_State#shipping_state.ships,
                      containers = Shipping_State#shipping_state.containers,
                      ports = Shipping_State#shipping_state.ports,
...
set_sail(Shipping_State, Ship_ID, {Port_ID, Dock}) ->
  ...
  DTaken = get_occupied_docks(Shipping_State, Port_ID),
  case not lists:member(Dock, Dtaken) of
    true ->
      NEW_PORT = lists:filter(fun (X) -> not lists:member(X, Port_ID) end, Port),

      #shipping_state{
                  ships = Shipping_State#shipping_state.ships,
                  containers = Shipping_State#shipping_state.containers,
                  ports = maps:(Port_ID, NEW_PORT, Shipping_State#shipping_state.ship_locations),
                  ship_locations = Shipping_State#shipping_state.ship_locations,
