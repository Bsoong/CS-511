		...
		false ->
			...
			receive
				{ChatPID, Ref, connect, History} ->
					...
					NewCons = maps:put(ChatName, ChatPID, State#cl_st.con_ch),
					NewState = #cl_st{
						gui = State#cl_st.gui,
...
do_leave(State, Ref, ChatName) ->
	case maps:is_key(ChatName, State#cl_st.con_ch) of
		...
		true ->
			...
			receive
				{ChatPID, Ref, ack_leave} ->
					%update con_ch's then send leave to gui
					NewCons = maps:remove(ChatName, State#cl_st.con_ch),
					NewState = #cl_st{
						gui = State#cl_st.gui,
						nick = State#cl_st.nick,
						con_ch = NewCons
